...
export class RevAndClRepoImpl implements IRevAndClRepo {
  ...
  async getAgentEmailByAgencyId<T>(aParams: IRevReqDto): Promise<T> {
    try {
      const db = await RevAndClRepoImpl.getDbInstance();
      const data = await db.all(
        `SELECT agn.email,ag.id from rev_db.agencies as ag  JOIN rev_db.agencyAgents  as age ON ag.id=age.agencyId JOIN rev_db.agents as agn ON age.agentId= agn.id where ag.id=${aParams.id}`
      );
      return data as T;
    } catch (error) {
      return Promise.reject(error);
